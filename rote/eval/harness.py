...
from rote.bootstrap.system import (
    AGENT_SEED,
    CompiledSystem,
    CountingModel,
...
from rote.compiler.registry import PlanRegistry
...
from rote.runtime.shadow import live_calls
...
def _live_arm(
    ...
    for exception in exceptions:
        ...
        trajectory, calls = run_live(exception, live_toolbox(gate, exception), model)
...
def _rote_attempt(
    ...
    result = execute_plan(
        ...
        toolbox=gate.for_task(
            policy_context(exception.exception_id, ExecutionPath.COMPILED_PLAN, plan.category)
        ),
    ...
    trajectory, calls = run_live(
        exception,
        live_toolbox(gate, exception),
        model,
    ...
    for task in tasks:
        ...
        for index in range(repeats):
            ...
            result = execute_plan(
                ...
                task_input=facts,
                toolbox=gate.for_task(
                    policy_context(task, ExecutionPath.COMPILED_PLAN, plan.category)
                ),
                inspector=Guard(config=default_guard_config()),
            )
            records.append(
...
def _replay_arm(
    ...
    for record in compiled:
        ...
        result = execute_plan(
            ...
            task_input=exception.facts.model_dump(mode="json"),
            toolbox=gate.for_task(
                policy_context(record.task_id, ExecutionPath.COMPILED_PLAN, plan.category)
            ),
            inspector=Guard(config=default_guard_config()),
        )
